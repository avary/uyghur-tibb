...
<template><div v-if="topics.length" class="saved-topics card"><h3>📚 ساقلانغان كىتاب بۆلەكلىرى</h3><RouterLink v-for="topic in topics" :key="topic.id" :to="topic.path" class="saved-topic"><b>{{ topic.title }}</b><small>{{ topic.bookTitle }} · {{ topic.startPage }}–{{ topic.endPage }}-بەت</small></RouterLink></div></template>
