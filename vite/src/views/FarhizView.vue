...
const q = ref(''); const topicQ = ref(''); const selected = ref(null); const reader = ref(null); const remoteTopics = ref([]); const api = useApi()
const route = useRoute()
const pages = computed(() => searchFarhiz(q.value))
const sections = computed(() => farhizSections().filter(section => remoteTopics.value.find(item => item.topic_id === section.id)?.review_status !== 'rejected').map(section => { const remote = remoteTopics.value.find(item => item.topic_id === section.id); return remote ? { ...section, title: remote.title || section.title, summary: remote.summary || section.summary || '' } : section }))
...
const sectionIndex = computed(() => sections.value.findIndex(section => section.id === selected.value?.id))
...
onMounted(async () => { const rows = await api.getBookTopics(); if (Array.isArray(rows)) { remoteTopics.value = rows.filter(item => item.book_id === FARHIZ_BOOK.id); const section = sections.value.find(item => item.id === route.query.section); if (section) openSection(section) } })
</script>
<template>
