...
const q = ref(''); const topicQ = ref(''); const selected = ref(null); const reader = ref(null)
...
<template>
  <section v-if="FARHIZ_BOOK">
    ...
    <div v-if="!q && sections.length" class="toc card"><b>بۆلەك يولباشچىسى ({{ visibleSections.length }})</b><input v-model="topicQ" class="input" type="search" placeholder="بۆلەك نامىدىن ئىزدەش..." aria-label="بۆلەك نامىدىن ئىزدەش"><button v-for="s in visibleSections" :key="s.id" :class="'level-' + s.level" @click="openSection(s)">{{ s.title }} <small>{{ s.startPage }}–{{ s.endPage }}-بەت</small></button></div>
