...
function progressKey() { return `uytibb_book_quiz_${book.value?.id}_${route.query.section || 'all'}` }
function loadBest() { try { best.value = Number(localStorage.getItem(progressKey()) || 0) } catch {} }
function saveBest() { if (score.value > best.value) { best.value = score.value; localStorage.setItem(progressKey(), String(best.value)) } }
